...
class PhysicsConstrainedLoss(nn.Module):
    ...
    def _physics_loss(self, f_pred, t_span, k_jmak, n_jmak, f_eq):
        B, T = f_pred.shape
        t_np = t_span.detach()
        f_jmak = f_eq.squeeze(-1).unsqueeze(1) * (1 - torch.exp(-k_jmak.unsqueeze(1) * t_np.unsqueeze(0).pow(n_jmak.unsqueeze(1))))
        f_jmak = f_jmak.clamp(0, 1)
        mask = t_span < t_span[-1] * 0.5
        if mask.sum() > 0:
