...
class Trainer:
    ...
    def _train_epoch(self, loader, epoch):
        ...
        for step, batch in enumerate(loader):
            ...
            if (step + 1) % self.mc.accumulate_grad_batches == 0:
    ...
    def _validate(self, loader):
        ...
        for batch in loader:
            ...
            try:
                f_pred = self.model(static, f_eq, dG, t_span)
                ml = min(f_pred.shape[1], f_true.shape[1])
                loss_d = self.criterion(f_pred[:, :ml], f_true[:, :ml], f_eq, t_span[:ml], k_j, n_j)
                for k in totals:
                    if k in loss_d:
...
def create_data_loaders(train_df, val_df, config=None):
    ...
    train_loader = DataLoader(train_ds, batch_size=config.model.batch_size, shuffle=True, collate_fn=_collate_fn, drop_last=True, num_workers=0, pin_memory=config.device.type == 'cuda')
