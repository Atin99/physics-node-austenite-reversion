...
def plot_kinetic_curves_with_uq(predictions, config=None):
    ...
    for i, (label, d) in enumerate(predictions.items()):
        ...
        if 'f_RA_true_obs' in d and d['f_RA_true_obs'] is not None:
            ax.scatter(d['t_hours_obs'], d['f_RA_true_obs'], color=color, s=20, marker='o', zorder=5, edgecolors='white', linewidths=0.5)
        elif 'f_RA_true' in d and d['f_RA_true'] is not None:
...
def plot_parity(f_true, f_pred, f_pred_std=None, config=None):
    ...
    upper = max(float(np.max(f_true)), float(np.max(f_pred))) * 1.1
    lim = [0, max(upper, 1e-3)]
    ...
    ax.text(0.05, 0.95, f'RMSE={rmse:.4f}\nR^2={r2:.4f}', transform=ax.transAxes, fontsize=8, va='top',
...
def plot_phase_diagram_section(config=None):
    ...
    c = config.visualization.colors
    ax.fill_between(Mn_r, Ac1s, Ac3s, alpha=0.3, color=c[2], label='alpha + gamma')
    ax.fill_between(Mn_r, Ac3s, 950, alpha=0.15, color=c[5], label='gamma')
    ax.fill_between(Mn_r, 400, Ac1s, alpha=0.15, color=c[1], label='alpha')
...
def plot_training_history(history, config=None):
    ...
    if 'val_rmse' in history and history['val_rmse']:
        axes[1, 1].cla()
        eps = range(1, len(history['val_rmse']) + 1)
        axes[1, 1].plot(eps, history['val_rmse'], color=c[0], alpha=0.8, label='Val RMSE')
        if 'val_real_rmse' in history and history['val_real_rmse']:
            axes[1, 1].plot(eps, history['val_real_rmse'], color=c[1], alpha=0.8, label='Val real RMSE')
        axes[1, 1].set_xlabel('Epoch')
        axes[1, 1].set_ylabel('RMSE')
        axes[1, 1].set_title('Observed-Point Metrics', fontsize=9)
...
def fig0_real_data_overview(config=None):
    ...
    ax.set_xlabel('Annealing Temperature (deg C)')
