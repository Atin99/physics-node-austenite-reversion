...
def build_prediction_panels(model, dataset, sample_ids, config, n_samples):
    ...
    for sample_id in sample_ids:
        ...
        obs_mask = sample.get('obs_mask', torch.ones_like(sample['traj'])) > 0.5
        ...
        predictions[label] = {
            ...
            't_hours_obs': (sample['t_span'][obs_mask].cpu().numpy() / 3600.0),
            'f_RA_true_obs': sample['traj'][obs_mask].cpu().numpy(),
        }
    return predictions
...
def build_parity_arrays(model, dataset, config, max_curves):
    ...
    for sample in dataset.samples[:max_curves]:
        ...
        obs_mask = sample.get('obs_mask', torch.ones_like(sample['traj'])).cpu().numpy() > 0.5
        f_true_all.append(sample['traj'].cpu().numpy()[obs_mask])
        f_pred_all.append(pred[obs_mask])
...
def main():
    ...
    parser.add_argument('--split', type=str, default='val', choices=['train', 'val', 'test'], help='Dataset split used for parity and representative curves')
