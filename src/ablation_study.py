...
_ckpt_candidates = [
    _base / 'models' / 'stage2_extended_best.pt',
    _base / 'models' / 'stage2_fixed_best.pt',
    _base / 'models' / 'checkpoints' / 'physics_node_best.pt',
    _base.parent / 'models' / 'v4_stage2_extended_best.pt',
    _base.parent / 'models' / 'v4_stage2_fixed_best.pt',
    _base.parent / 'models' / 'v43_hifi_stage2_best.pt',
]
ckpt_path = next((c for c in _ckpt_candidates if c.exists()), None)
if ckpt_path is None:
    raise FileNotFoundError(f"No checkpoint found. Searched: {[str(c) for c in _ckpt_candidates]}")
...
hist_path = next((p for p in [
    Path(__file__).parent / 'models' / 'stage2_extended_history.csv',
    Path(__file__).parent.parent / 'models' / 'v4_stage2_extended_history.csv',
] if p.exists()), Path(__file__).parent / 'models' / 'stage2_extended_history.csv')
if hist_path.exists():
