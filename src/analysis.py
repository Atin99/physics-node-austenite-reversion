...
_PARENT = PROJECT.parent

def _find(candidates):
    """Return first existing path from candidates list."""
    return next((c for c in candidates if c.exists()), candidates[0])

DATA_CSV = _find([PROJECT / "data" / "literature_validation" / "literature_validation.csv",
                  _PARENT / "data" / "literature" / "literature_validation.csv"])
S1_HIST = _find([PROJECT / "models" / "stage1_history.csv",
                 _PARENT / "models" / "v4_stage1_history.csv"])
S2_FIXED_HIST = _find([PROJECT / "models" / "stage2_fixed_history.csv",
                       _PARENT / "models" / "v4_stage2_fixed_history.csv"])
S2_RUN7_HIST = _find([PROJECT / "models" / "stage2_run7_history.csv",
                      _PARENT / "models" / "v4_stage2_run7_history.csv"])
S2_FIXED_SUMMARY = _find([PROJECT / "models" / "stage2_fixed_run_summary.json",
                          _PARENT / "models" / "v4_stage2_fixed_summary.json"])
S2_RUN7_SUMMARY = _find([PROJECT / "models" / "stage2_run7_summary.json",
                         _PARENT / "models" / "v4_stage2_run7_summary.json"])
OUT_DIR = _PARENT / "results"
