    ...
    def _encode_state(self, state, cond):
        ...
        for layer in self.net:
            ...
            if isinstance(layer, nn.SiLU) and idx < len(self.film_layers):
                ...
                idx += 1
        # Keep the adaptive ODE solver away from extreme hidden activations.
        return torch.clamp(x, min=-12.0, max=12.0)
...
class PhysicsNODE(nn.Module):
    ...
    def forward(self, static_features, f_eq, delta_G_norm, t_span, lengths=None):
        ...
        trajectories = []
        for i in range(B):
            cur_len = max(1, min(lengths[i], max_len))
            traj_i = self._solve_trajectory(cond[i:i + 1], f_eq[i:i + 1], delta_G_norm[i:i + 1], t_span[i, :cur_len]).squeeze(0)
            if cur_len < max_len:
                pad = traj_i[-1:].expand(max_len - cur_len)
                traj_i = torch.cat([traj_i, pad], dim=0)
            trajectories.append(traj_i)
        return torch.stack(trajectories, dim=0)

    def predict_with_uncertainty(self, static_features, f_eq, delta_G_norm, t_span, n_samples=100, lengths=None):
        was_training = self.training
        self.train()
        preds = []
        for _ in range(n_samples):
            with torch.no_grad():
                pred = self.forward(static_features, f_eq, delta_G_norm, t_span, lengths=lengths)
                preds.append(pred)
        if not was_training:
            self.eval()
